...
export class ChatService {
  ...
  // ==================== NEW: TENANT CHAT CREATION FIXES ====================

  /**
   * Creates a chat between tenant and landlord
   * Since tenants don't have propertyId, we use unit.id instead
   */
  createTenantLandlordChat(): Observable<ApiResponse<ChatRoom>> {
    ...
    return this.tenantService.getTenantUnits().pipe(
      switchMap(response => {
        ...
        if (!response || response.success === false || !response.data || !Array.isArray(response.data) || response.data.length === 0) {
          return throwError(() => new Error('No units found. You may not have any assigned units.'));
        }
        
        const unit = response.data[0];
        console.log('🔍 Using unit:', {
          id: unit.id,
          unitNumber: unit.unitNumber,
          propertyName: unit.propertyName,
          allFields: Object.keys(unit)
        });
        
        // STRATEGY 1: Try unit.id as propertyId (most likely what was working before)
        console.log(`🔄 Strategy 1: Trying unit.id (${unit.id}) as propertyId`);
        return this.tryCreateChat(`/tenant/landlord/${unit.id}`, 'Strategy 1 (unit.id)').pipe(
          catchError(error1 => {
            console.log('❌ Strategy 1 failed:', error1.message);
            
            // STRATEGY 2: Look for propertyId in other fields
            const propertyId = this.findPropertyIdInUnit(unit);
            if (propertyId) {
              console.log(`🔄 Strategy 2: Found propertyId ${propertyId} in unit`);
              return this.tryCreateChat(`/tenant/landlord/${propertyId}`, 'Strategy 2 (propertyId)').pipe(
                catchError(error2 => {
                  console.log('❌ Strategy 2 failed:', error2.message);
                  return this.tryStrategy3(unit, 'landlord');
                })
              );
            }
            
            console.log('🔄 No propertyId found, moving to Strategy 3');
            return this.tryStrategy3(unit, 'landlord');
          })
        );
      }),
      catchError(error => {
        ...
        return throwError(() => new Error('Failed to create chat. Please try again or contact support.'));
      })
    );
  }
   ...
   * Uses the same approach as tenant-landlord chat
   */
    ...
    return this.tenantService.getTenantUnits().pipe(
      switchMap(response => {
        if (!response?.data || response.data.length === 0) {
          return throwError(() => new Error('No units found'));
        }
        
        const unit = response.data[0];
        
        // STRATEGY 1: Try unit.id as propertyId
        return this.tryCreateChat(`/tenant/caretaker/${unit.id}`, 'Strategy 1 (unit.id)').pipe(
          catchError(error1 => {
            // STRATEGY 2: Look for propertyId
            const propertyId = this.findPropertyIdInUnit(unit);
            if (propertyId) {
              return this.tryCreateChat(`/tenant/caretaker/${propertyId}`, 'Strategy 2 (propertyId)');
            }
            
            // STRATEGY 3: Try different variations
            return this.tryStrategy3(unit, 'caretaker');
          })
        );
      })
    );
  ...
  /**
   * Helper method to try creating chat with a specific endpoint
   */
  private tryCreateChat(endpoint: string, strategyName: string): Observable<ApiResponse<ChatRoom>> {
    console.log(`🔄 ${strategyName}: Calling ${endpoint}`);
    ...
    return this.http.post<ApiResponse<ChatRoom>>(
      ...
      catchError(error => {
        console.error(`❌ ${strategyName} FAILED:`, error.status, error.message);
  ...
  /**
   * Find propertyId in unit object (check various field names)
   */
  private findPropertyIdInUnit(unit: any): number | null {
    ...
    console.log('🔍 Searching for propertyId in unit...');
    
    // Check direct fields
    for (const field of possibleFields) {
      if (unit[field] !== undefined && unit[field] !== null) {
        console.log(`✅ Found ${field}: ${unit[field]}`);
        return Number(unit[field]);
      }
    }
    
    // Check nested property object
    if (unit.property) {
      for (const field of possibleFields) {
        if (unit.property[field] !== undefined && unit.property[field] !== null) {
          console.log(`✅ Found property.${field}: ${unit.property[field]}`);
          return Number(unit.property[field]);
        }
      }
      
      // Check property.id
      if (unit.property.id) {
        console.log(`✅ Found property.id: ${unit.property.id}`);
        return Number(unit.property.id);
      }
    }
    
    console.log('❌ No propertyId found in unit');
    return null;
  }

  /**
   * Strategy 3: Try different approaches
   */
  private tryStrategy3(unit: any, target: string = 'landlord'): Observable<ApiResponse<ChatRoom>> {
    console.log(`🔄 Strategy 3: Trying different approaches for ${target}`);
    
    // Try to extract ID from propertyName (e.g., "Smarty-1" → 1)
    const extractedId = this.extractIdFromPropertyName(unit.propertyName);
    if (extractedId) {
      console.log(`🔄 Extracted ID ${extractedId} from propertyName: ${unit.propertyName}`);
      return this.tryCreateChat(`/tenant/${target}/${extractedId}`, 'Strategy 3 (extracted ID)').pipe(
        catchError(error3 => {
          console.log('❌ Strategy 3a failed');
          return this.tryFinalFallback(unit, target);
        })
      );
    }
   ...
   */
  private tryFinalFallback(unit: any, target: string): Observable<ApiResponse<ChatRoom>> {
    console.log(`🔄 Final fallback for ${target}`);
    
    // Try with a default/hardcoded ID (LAST RESORT)
    const defaultPropertyId = 1; // Common default
    
    return this.tryCreateChat(`/tenant/${target}/${defaultPropertyId}`, `Final (default ID ${defaultPropertyId})`).pipe(
      catchError(finalError => {
        console.error(`❌ ALL STRATEGIES FAILED for chat with ${target}`);
        
        // User-friendly error message
        let errorMessage = `Cannot create chat with ${target}. `;
        
        if (finalError.status === 404) {
          errorMessage += `The system cannot find your property. `;
          errorMessage += `Your property name is: "${unit.propertyName}". `;
          errorMessage += `Please contact your ${target} and ask them to start the chat from their side.`;
        } else if (finalError.status === 401 || finalError.status === 403) {
          errorMessage += `You may not have permission to create this chat.`;
        } else {
  ...
  /**
   * Extract numeric ID from property name (e.g., "Smarty-1" → 1)
   */
  private extractIdFromPropertyName(propertyName: string): number | null {
    if (!propertyName) return null;
    
    // Look for numbers in the name
    const matches = propertyName.match(/\d+/g);
    ...
    return propertiesArray
      .map((item: any) => {
        ...
        const processedProperty: Property = {
          ...
          ownerName: propertyData.landlordName || propertyData.ownerName || '',
  ...
  // Keep the original methods for landlords and caretakers (they have propertyId)
 ...
 deleteMessage(messageId: number): Observable<ApiResponse> {
  ...
  return this.http.delete<ApiResponse>(`${this.apiUrl}/messages/${messageId}`, { // ← Added closing backtick here
    headers: this.getHeaders() 
  }).pipe(
    timeout(15000),
    tap(response => {
